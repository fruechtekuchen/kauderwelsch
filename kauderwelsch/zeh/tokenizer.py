...
class Tokenizer:
    ...
    def expect(self, pattern, ignore_whitespace: bool = True) -> re.Match:
        if m := self.match(pattern, ignore_whitespace):
            return m
        raise TokenizationError(f"Expected {pattern!r}")

    def match(self, pattern, ignore_whitespace: bool = True) -> re.Match:
